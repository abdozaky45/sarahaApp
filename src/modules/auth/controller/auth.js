...
export const newConfirmEmail = asynchandler(async (req, res, next) => {
  ...
  if (user.confirmEmail) {
    return next(new Error("/login", { cause: 200 }));
  }
  const newTokenConfirmEmail = jwk.sign(
    { id: user._id, email: user.email },
    process.env.EMAIL_SIGNATURE,
    { expiresIn: 60 * 2 }
  );
  const linkCofirmEmail = `${req.protocol}://${req.headers.host}/auth/confirmEmail/${newTokenConfirmEmail}`;
  const html = `<!DOCTYPE html>
                <html>
                <head>
                    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css"></head>
  ...
  await sendEmail({ to: user.email, subject: "newConfirmEmail", html });
...
export const login = asynchandler(async (req, res, next) => {
  ...
  const isOnline = await userModel.findByIdAndUpdate(user._id, {
    $set: { isOnline: true },
  });
  return res
    .status(200)
...
export const changePassword = asynchandler(async (req, res, next) => {
  ...
  if (!match) {
    return next(new Error("The password is wrong", { cause: 400 }));
  }
  const hashPassword = bcrypt.hashSync(
    newPassword,
    parseInt(process.env.saltRounds)
